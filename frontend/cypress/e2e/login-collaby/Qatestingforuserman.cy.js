describe('should be able to do usermanagement things', () => {
  ...
  it('should be able to create a new user(Manager)', () => {
    ...
    cy.get('.relative > .w-full').type(EmployeeID.substring(3, EmployeeID.length))
  ...
  it('should be able to create a new user(frontline)', () => {
     cy.login('test@example.com', '12345')
    ...
    cy.get('#role').select('Frontline')

    cy.get('#supervisor').find('option').then(options => {
        const count = options.length;
        // Skip the first option if it's a placeholder
        const randomIndex = count > 1 ? Math.floor(Math.random() * (count - 1)) + 1 : 0;
        const randomValue = options[randomIndex].value;
        cy.get('#supervisor').select(randomValue);
    });
  ...
  it('should be able to create a new user(Support)', () => {
     cy.login('test@example.com', '12345')
  ...
  it('should be able to create a new user(Admin)', () => {
     cy.login('test@example.com', '12345')
